...
export default function Teams() {
  ...
  const avatar = (team) => {
    ...
    if (logo) return <img src={logo} alt={team.name} style={{ width: 44, height: 44, borderRadius: 8, border: '1px solid #2e4370', objectFit: 'cover' }} />
    const initials = team.name.split(' ').map(s => s[0]).slice(0,2).join('').toUpperCase()
    return (
      <div style={{ width: 44, height: 44, borderRadius: 8, border: '1px solid #2e4370', background: '#0e1b34', display: 'grid', placeItems: 'center', fontWeight: 700 }}>
        {initials}
      </div>
    )
  }
